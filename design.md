...
```mermaid
graph TD
    A[点击消息重写按钮] --> B[弹窗展示多行编辑框]
    B --> C[用户确认提交]
    C --> D[POST /api/rewrite]
    D --> E[前端更新该消息内容]
```

**重写交互优化**: 为了提升长文本的编辑体验，重写功能将采用一个独立的弹窗，其中包含一个**多行文本域 (textarea)**。该文本域支持根据内容**自动调整高度**，方便用户查看和编辑完整的消息。

**重要说明**: 重写功能将仅修改指定消息的内容，而不会清除该消息之后的所有后续消息。

#### 4.3 全局编辑功能 (Global Edit)
为了提供更灵活的对话历史管理，将引入全局编辑功能。

- **入口**：在主对话区右侧（例如，与“控制栏”对齐的位置）增加一个“全局编辑”按钮。
- **界面**：点击按钮后，主对话区右侧将滑出一个类似于 VSCode 的编辑器窗口。
  - 该编辑器窗口将加载当前对话的完整 Markdown 内容（即 `data/conversations/{user_id}/{conv_id}.md` 文件的内容）。
  - 编辑器应支持 Markdown 语法高亮。
- **交互**：
  - **保存**：用户可以在编辑器中直接编辑内容，并通过 `Ctrl + S` 快捷键或点击编辑器内的“保存”按钮进行保存。保存操作将调用后端 API 更新对话文件。
  - **发送给大模型**：编辑器内提供一个“发送”按钮，点击后将当前编辑器中的内容作为新的对话历史，并触发一次新的大模型生成请求。这相当于一次特殊的“重写”操作，但作用于整个文件。
  - **隐藏**：在编辑器窗口的相同位置（或顶部），提供一个“隐藏”按钮，点击后编辑器窗口滑回，主对话区恢复全宽显示。
- **技术实现考虑**：
  - 前端编辑器组件：考虑使用 Monaco Editor 或其他轻量级 Markdown 编辑器。
  - 后端 API：需要一个 `PUT /api/conversations/{conv_id}` 或类似的接口来接收并保存完整的 Markdown 内容。
  - 大模型交互：发送给大模型时，后端需要解析提交的 Markdown 内容，提取最新的消息作为输入，并根据整个历史生成回复。
